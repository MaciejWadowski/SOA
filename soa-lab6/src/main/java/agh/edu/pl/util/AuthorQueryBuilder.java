...
public class AuthorQueryBuilder {
    ...
    public class Builder {
        ...
        public Builder () {
            ...
            query.append("SELECT DISTINCT a FROM Author a JOIN Book b on b.author=a ");
        }

        public Builder complexQueryBuilder () {
            query.append("JOIN Borrow bb on bb.book = b JOIN User u on bb.user = u WHERE 1=1");
            return this;
        }
        ...
        public Builder applyAuthorFilter(QueryFilterType type, String input) {
            if (input != null && !input.isBlank()) {
                query.append(" AND a.name ").append(operatorConstruct(type, ":author", null));
            }
            return this;
        }
